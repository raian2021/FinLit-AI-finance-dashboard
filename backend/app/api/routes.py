...
async def stats(db: AsyncSession = Depends(get_db)):
    ...
    latest = await db.scalar(select(func.max(Transaction.transaction_date)))
    return {
...
async def upload_csv(
    ...
    return await import_transactions(db, content, bank_enum)


@router.get("/analytics/cashflow", response_model=CashFlowTruth)
async def cashflow(months: int = Query(3, ge=1, le=12), db: AsyncSession = Depends(get_db)):
    end = date.today()
    start = end - timedelta(days=30 * months)
    return await get_cashflow_truth(db, start, end)


@router.get("/analytics/periods")
async def periods(db: AsyncSession = Depends(get_db)):
...
async def monthly(year: int, month: int, db: AsyncSession = Depends(get_db)):
    if not (1 <= month <= 12) or not (2000 <= year <= 2100):
        raise HTTPException(400, "Invalid year/month")

    overview = await get_monthly_overview(db, year, month)
    if not overview:
...
async def insights(req: InsightRequest, db: AsyncSession = Depends(get_db)):
    summary = await build_ai_summary(db, months=req.months)
